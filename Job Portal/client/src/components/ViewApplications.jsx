          ...
          <tbody>
            {
              viewApplicationsPageData.map((user,index)=>(
                <tr className='border-b border-gray-300 text-gray-700' key={index}>
                 ...
                 <td className='px-4 py-2 text-center max-sm:hidden'>{user.jobTitle} </td>
                 <td className='px-4 py-2 text-center max-sm:hidden'>{user.location} </td>
                 <td className='px-4 py-2 text-center'><span className='text-sm text-blue-600 bg-blue-50 rounded px-3 py-2 flex items-center justify-center gap-1'>Resume <img src={assets.resume_download_icon} alt="download" /></span></td>
                 <td className='relative px-4 py-2 text-center group text-2xl'>... <div className='absolute right-0 md:left-0 top-0 hidden z-10 p-2 group-hover:flex flex-col gap-2 border border-gray-400 rounded bg-white'>
